...
from frcnn.batch_normalization import FixedBatchNormalization
...
def conv_block_td(input_tensor, kernel_size, filters, stage, block, input_shape, strides=(2, 2), trainable=True):
    """ Convolutional block time distributed """
...
def nn_base(input_tensor=None, trainable=False):
    """ Resnet50 from [Deep Residual Learning for Image Recognition](https://arxiv.org/abs/1512.03385)

        Args:
        input_tensor: image
        trainable: False by default, True during training
...
def classifier_layers(x, input_shape, trainable=False):
    """ Standard classifier layer with two fully-connected layers """
...
def rpn(base_layers, num_anchors):
    ...
    Args:
        base_layers: resnet network that generates feature map
        num_anchors: 9
...
def classifier(base_layers, input_rois, num_rois, nb_classes=2):
    """ R-CNN Classifier/Regressor network with two fully-connected layers followed by a softmax-activate fully-connected layer
        for classification and a linear-activated layer for bounding box regression.

        Args:
        base_layers: resnet network that generates feature map
        input_rois: empty RoI, for the shape
        nb_classes: only two classes present, gate and backgroung

    Returns:
        [out_class, out_regr]
        out_class: classification for whether it's a gate or background
        out_regr: bboxes regression
    """
